...
def test_gifti_to_array_both_hemispheres(mock_gifti_func):
    ...
    array, split_idx = gifti_to_array(
        left_gifti=mock_gifti_func,
        right_gifti=mock_gifti_func
    )
    
    assert isinstance(array, np.ndarray)
    assert isinstance(split_idx, int)
    assert split_idx == 100  # Number of vertices in mock data
    assert array.shape == (200,)  # Combined vertices from both hemispheres
...
def test_array_to_gifti_single():
    ...
    assert len(gifti_img.darrays) == array.shape[0]
